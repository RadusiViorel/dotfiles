...
cyclefonts(const Arg *arg)
{
	...
	{
		char cmd[512];
		/* extract family name (up to first colon) for a clean notification */
		char name[256];
		int i;
		for (i = 0; usedfont[i] && usedfont[i] != ':' && i < 255; i++)
			name[i] = usedfont[i];
		name[i] = '\0';
		snprintf(cmd, sizeof cmd, "notify-send -t 2000 'st font' '%s' &", name);
		system(cmd);
	}
}
...
xmakeglyphfontspecs(XftGlyphFontSpec *specs, const Glyph *glyphs, int len, int x, int y)
{
	...
	for (i = 0, xp = winx, yp = winy + font->ascent + win.cyo; i < len; ++i) {
		...
		/* Boxdraw: store shape index in font pointer, skip font lookup. */
		if (mode & ATTR_BOXDRAW) {
			specs[numspecs].font = (XftFont *)(uintptr_t)boxdrawindex(&glyphs[i]);
			specs[numspecs].glyph = 0;
...
xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len, int x, int y, int dmode)
{
	...
	if (dmode & DRAW_FG) {
		...
		if (base.mode & ATTR_UNDERLINE || base.mode & ATTR_URL) {
			/* Resolve underline color from decor field */
			Color *ulcolor = fg;
			Color truefulcolor;
			...
			if (decorcolor != DECOR_DEFAULT_COLOR &&
				...
				if (IS_TRUECOL(decorcolor)) {
					XRenderColor rc;
					rc.red   = TRUERED(decorcolor);
					rc.green = TRUEGREEN(decorcolor);
					rc.blue  = TRUEBLUE(decorcolor);
					rc.alpha = 0xffff;
					XftColorAllocValue(xw.dpy, xw.vis, xw.cmap, &rc, &truefulcolor);
				} else if (xw.depth == 32) {
					XRenderColor rc;
					rc.red   = dc.col[decorcolor].color.red;
					rc.green = dc.col[decorcolor].color.green;
					rc.blue  = dc.col[decorcolor].color.blue;
					rc.alpha = dc.col[decorcolor].color.alpha;
					XftColorAllocValue(xw.dpy, xw.vis, xw.cmap, &rc, &truefulcolor);
				} else {
					truefulcolor = dc.col[decorcolor];
				}
				ulcolor = &truefulcolor;
			}

			int ustyle = tgetdecorstyle(&base);
			if (ustyle == UNDERLINE_CURLY) {
				int uly = winy + win.cyo + dc.font.ascent + gap;
				uly -= MAX(0, uly + thick - (winy + win.ch));
				int ulh = (winy + win.ch) - uly;
				xdrawundercurl(xw.draw, ulcolor, winx, uly, width, ulh, thick);
			} else {
				int liney = winy + win.cyo + dc.font.ascent + gap;
				liney -= MAX(0, liney + thick - (winy + win.ch));
				int linethick = (ustyle == UNDERLINE_DOUBLE) ? MAX(1, thick / 2) : thick;
				XftDrawRect(xw.draw, ulcolor, winx, liney, width, linethick);
				if (ustyle == UNDERLINE_DOUBLE)
					XftDrawRect(xw.draw, ulcolor, winx, liney + linethick * 2, width, linethick);
			}
		}

		/* Render the glyphs. */
		if (base.mode & ATTR_BOXDRAW) {
			drawboxes(winx, winy + win.cyo, win.cw, win.ch, fg, bg, specs, len);
		} else {
...
xdrawimages(Glyph base, Line line, int x1, int y1, int x2) {
	...
	for (int x = x1; x < x2; ++x) {
		...
		if (!tgetimgrow(g)) tsetimgrow(g, cur_row);
		if (!tgetimgcol(g) && (cur_col & ~0x1ff) == 0) tsetimgcol(g, cur_col);
		if (!tgetimgid4thbyteplus1(g)) tsetimg4thbyteplus1(g, cur_id_4thbyteplus1);
	}
	uint32_t image_id = image_id_24bits;
	if (last_id_4thbyteplus1)
		image_id |= (last_id_4thbyteplus1 - 1) << 24;
...
xdrawline(Line line, int x1, int y1, int x2)
{
	 ...
	 * This prevents wide glyphs from being truncated. */
...
cachemonitorinfo(void)
{
	...
	for (int i = 0; i < monitors_num; ++i) {
		...
		for (int j = 0; j < (int)LEN(monitors_config); ++j)
