...
followurl(int x, int y) {
	...
	/* prepend https:// for bare www. URLs */
	char *url = linestr + url_start;
	char fullurl[1600];
...
tdefcolor(const int *attr, int *npar, int l)
{
	...
	case 2: /* direct color in RGB space */
		...
		/* Support both 4-arg form (2;r;g;b) and 6-arg colon form
		 * (2::cs:r:g:b where cs is colorspace, typically empty/0).
		 * Detect the 6-arg form by checking if there are 5 more args. */
		if (*npar + 5 < l) {
			/* 6-arg form: skip colorspace at [+2], use [+3],[+4],[+5] */
			r = attr[*npar + 3];
			g = attr[*npar + 4];
			b = attr[*npar + 5];
			*npar += 5;
...
csihandle(void)
{
	...
	case 'u': /* DECRC -- Restore cursor position (ANSI.SYS) */
		if (csiescseq.priv) {
			/* ESC[?u -- kitty keyboard protocol query: respond no flags */
			ttywrite("\033[?0u", 5, 0);
		} else {
			tcursor(CURSOR_LOAD);
		}
	...
	case '>':
		...
		case 'q': /* XTVERSION -- Print terminal name and version */
			...
			ttywrite(buf, len, 0);
			break;
		case 'm': /* xterm modifyOtherKeys -- silently ignore */
			break;
		default:
			goto unknown;
		}
	...
	case '$': /* DECRQM -- Request DEC private mode */
		if (csiescseq.mode[1] == 'p') {
			/* Respond "not recognized" (Ps;0$y) for all queried modes */
	...
	case 't': /* XTWINOPS -- Window manipulation */
		...
		case 18: /* Report the size of the text area in characters. */
			...
			ttywrite(buf, len, 0);
			break;
		case 22: /* Save window title -- no-op (we don't stack titles) */
		case 23: /* Restore window title -- no-op */
			break;
		default:
			goto unknown;
		}
...
acmpl_begin:
    ...
    if (acmpl_status == EOF) {
        /* full cycle completed — restore original text and deactivate */
        active = 0;
        pclose(acmpl_exec);
        acmpl_exec = NULL;
        unlink(stbuffile);
